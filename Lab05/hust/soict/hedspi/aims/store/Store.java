...
public class Store {
    ...
    public void addMedia(Media media) throws DuplicateMediaException, LimitExceededException {
        if (isDuplicateMedia(media)) {
            throw new DuplicateMediaException("Duplicate media found: " + media.getTitle());
        } else if (itemsInStore.size() >= STORE_LIMIT) {
            throw new LimitExceededException("Store limit exceeded. Cannot add more media.");
        } else {
    ...
    private boolean isDuplicateMedia(Media media) {
